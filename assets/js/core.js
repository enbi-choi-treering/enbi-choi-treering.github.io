...
(function initSheetsLoader() {
  if (!SHEETS_URL || SHEETS_URL === 'YOUR_URL') return;
...
/* ---- Date formatter (used by all pages) ---- */
/*
 * Input : "2025", "2025-04", or "2025-04-15"
 * Output: "2025", "Apr 2025", or "Apr 15, 2025"
 * Used  : anywhere a date/year is displayed across all pages.
 */
function formatDate(d) {
  ...
  if (p.length < 2 || !p[1]) return p[0];           // "2025"
  const mo = MONTHS[parseInt(p[1], 10) - 1] || p[1];
  if (p.length === 2) return `${mo} ${p[0]}`;        // "Apr 2025"
  return `${mo} ${parseInt(p[2], 10)}, ${p[0]}`;     // "Apr 15, 2025"
}
